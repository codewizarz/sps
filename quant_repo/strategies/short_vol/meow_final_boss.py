#!/usr/bin/env python3
"""
MEOW FINAL BOSS STRATEGY
Frozen version from Iteration 2
Performance:
- Return: ~63.98%
- Max DD: ~7.83%
- Sharpe: ~3.06
...
WARNING:
This is a high-performance backtest candidate.
Must pass validation before live trading.
...
class RegimeFilter:
    """Classifies volatility regime and translates it into sizing behavior."""
    ...
    def assess(
        ...
        if rv20 < config.low_vol_threshold:
            regime = VolatilityRegime.LOW
            size_multiplier = 1.0
        elif rv20 <= config.high_vol_threshold:
        ...
        notes: List[str] = [f"RV={rv20 * 100:.1f}"]
...
class QualityFilter:
    """Ensures ATM straddles are tradable and not structurally thin."""
    ...
    def check(
        ...
        if snapshot.spread_proxy_pct > config.max_spread_proxy_pct:
            blockers.append(f"Spread proxy {snapshot.spread_proxy_pct:.1%}")

        premium_floor = max(
            config.min_premium_absolute,
            snapshot.spot * config.min_premium_yield_pct,
        )
        ...
        if blockers:
            return False, quality_score, "; ".join(blockers)
        return True, quality_score, "ATM quality passed"


class PositionSizer:
    """Adaptive lot sizing linked to realized volatility and portfolio state."""
    ...
    def compute_lots(
        ...
        vol_scale = float(np.clip(vol_scale, config.min_position_fraction, config.max_position_fraction))
        ...
        raw_scale = min(vol_scale * adjustment_scale, config.max_position_fraction)
        final_scale = max(config.min_position_fraction, raw_scale)
        final_scale *= corr_scale
        final_scale = min(final_scale, config.max_position_fraction)
...
class BacktestEngine:
    ...
    def _build_candidate(
        ...
        if iv_jump_prev_day > self.config.event_day_iv_jump_threshold:
            logger.debug(
        ...
        quality_ok, quality_score, quality_reason = QualityFilter.check(
            snapshot,
            self.config,
        )
        if not quality_ok:
            logger.debug(
                f"  SKIP {entry_date.date()} {context.symbol}: quality | {quality_reason}"
            )
            return None
    ...
    def _process_entries(
        ...
        if self.cooldown_cycles_remaining > 0:
            logger.info(
                f"  SKIP {current_date.date()}: cooldown active for next cycle"
            )
